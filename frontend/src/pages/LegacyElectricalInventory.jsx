...
export default function LegacyElectricalInventory() {
  const navigate = useNavigate();
  const { electricalInventory, fetchElectricalInventory } = useStore();
  ...
  const filteredItems = (electricalInventory || []).filter((item) => {
  ...
  return (
    <div className="bg-slate-100 min-h-screen text-slate-800 transition-colors duration-300">
      ...
      <div className="ml-64 p-6">
        ...
        <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
          <div>
            <button
              ...
              className="flex items-center gap-2 text-sm font-semibold text-slate-550 hover:text-amber-600 transition mb-2 group cursor-pointer"
            >
            ...
            <p className="text-slate-550 text-sm mt-1">
              Electrical item registrations, variants, balances, and purchase orders history.
            </p>
          </div>
          <button
        ...
        {/* Metric Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 no-print">
          <motion.div
          ...
          >
            <div>
              <p className="text-slate-450 text-xs font-bold uppercase tracking-wider">Total Orders / Rows</p>
          ...
          >
            <div>
              <p className="text-slate-450 text-xs font-bold uppercase tracking-wider">Total Cost / Value</p>
          ...
          >
            <div>
              <p className="text-slate-450 text-xs font-bold uppercase tracking-wider">Dealers / Vendors</p>
              <h3 className="text-2xl font-extrabold text-blue-650 mt-1">{uniqueDealers}</h3>
            </div>
        ...
        <div className="bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between no-print">
          <div className="relative w-full sm:max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by name, variant, dealer, bill #..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 outline-none text-sm font-medium focus:border-amber-500 transition-colors"
            />
          </div>
          <div className="flex items-center gap-2 self-end sm:self-auto text-xs font-semibold text-slate-550">
            <span>Show</span>
            <select
              value={itemsPerPage}
            ...
            <table className="w-full text-left border-collapse table-auto min-w-[1200px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-550 text-xs font-bold uppercase tracking-wider">
                  ...
                  <th className="p-4 w-28">Variant / Qty Text</th>
                  ...
                  <th className="p-4 min-w-[220px]"><div className="flex items-center gap-1.5"><FaBuilding className="text-slate-400" /> Dealer / Supplier</div></th>
                  <th className="p-4 w-20 text-center">SLP</th>
                ...
                {currentItems.length > 0 ? (
                  currentItems.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-4 text-slate-700 font-medium">
                        <span className="bg-amber-50 text-amber-700 border border-amber-100/50 px-2 py-0.5 rounded text-xs">
                          {item.variant ?? "—"}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-right text-indigo-650 font-bold">
                        {item.balance ?? "—"}
                      </td>
                      <td className="p-4 text-slate-600 leading-normal text-xs font-medium">
                        {item.dealer_name ?? "—"}
                      </td>
                      <td className="p-4 text-center font-mono text-slate-550 text-xs">
                        {item.slp ?? "—"}
                      </td>
                  ...
                  <tr>
                    <td colSpan={15} className="p-12 text-center text-slate-400 font-semibold italic">
                      No electrical inventory items found.
                    </td>
                  </tr>
                )}
              </tbody>

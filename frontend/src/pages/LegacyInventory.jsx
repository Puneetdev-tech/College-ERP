...
export default function LegacyInventory() {
  ...
  const { legacyInventory, fetchLegacyInventory } = useStore();
  ...
  // Filter legacy inventory based on search query
  const filteredItems = (legacyInventory || []).filter((item) => {
      ...
      <div className="ml-64 p-6">
        ...
        <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
          <div>
            <button
              ...
              className="flex items-center gap-2 text-sm font-semibold text-slate-550 hover:text-blue-600 transition mb-2 group cursor-pointer"
            >
            ...
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
              ...
              Legacy CSV Store Records
            </h1>
            <p className="text-slate-550 text-sm mt-1">
              Historical ledger items imported directly from the store's primary CSV archive.
            </p>
          </div>
          <button
            onClick={() => window.print()}
            className="bg-purple-600 hover:bg-purple-750 text-white font-semibold px-5 py-3 rounded-xl cursor-pointer transition shadow hover:shadow-lg active:scale-95 flex items-center gap-2 text-sm"
          >
            <FaFileInvoice />
        ...
        <div className="bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between no-print">
          <div className="relative w-full sm:max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by name, dealer, bill #, or remarks..."
              value={search}
            ...
            <table className="w-full text-left border-collapse table-auto min-w-[1200px]">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {currentItems.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={14} className="p-12 text-center text-slate-400 font-semibold italic">
                      No legacy inventory items matched your search query.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="bg-slate-50/70 border-t border-slate-100 p-4 flex items-center justify-between no-print">
              ...
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-600">
                ...
                {Array.from({ length: totalPages }).map((_, idx) => {
                  const pNum = idx + 1;
                  // Render active page, and window of pages around active page
                  if (
                    pNum === 1 ||
                    pNum === totalPages ||
